package com.javapai.thirdparty.haiguan.service;
...
import com.javapai.thirdparty.haiguan.HaiguanController;
import com.javapai.thirdparty.haiguan.HaiguanSign;
...
public class HaiguanRepayService {
	...
	public String repDataApply(HgCheckVO entity) {
		...
		CompletableFuture.supplyAsync(() -> {
			...
			try {
				...
				if(HttpStatus.SC_OK == response.getStatusLine().getStatusCode()) {
					...
					logger.info("-----海关179数据上报结果:{}", result);
				} else {
					logger.info("-----海关179数据上报异常:{}", JSON.toJSONString(response));
				}
